...
{
/* Esta es la que estima todo. ¡¡¡¡x tiene que estar ordenado!!!! */
  ...
  if (cn == 0)
    {
      printf("Warning -- interquartile range is 0\n");
      return(-1);
    }
 ...
 /*     printf("alfa: %lf beta: %lf c: %lf d: %lf z: %lf",
  ...
  switch (symm)
    {
      case 0:
        ...
        *zeta = q50 + *c * sign * (*zeta);
        ...
        //  delta = *zeta;
        //else
        //  delta = *zeta - *beta * (*c) * tan(M_PI*(*alpha) / 2);
        break;
      case 1:
        for(i = 1;i<15;i++)
          {
...
{
//devuelve los estadisticos para c y zeta
  if (cn!=NULL) *cn = frctl(x,.75,n) - frctl(x,.25,n);
  if (zn!=NULL) *zn = frctl(x,.5 ,n);
  return;
}

void czab(double alfa, double beta, double cn, double q50,
          double *c, double *zeta)
{
//Estima c y zeta a partir de alfa, beta y de los estadisticos cn y zn (la mediana q50)
  double s,t;
  int i,j,sign;
