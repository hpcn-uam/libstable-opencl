...
int main(int argc, char *argv[])
{
	...
	if (argc > 1) {
		param = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else param = 0;

	if (argc > 1) {
		xmin = strtod(argv[i], &aux);
		argc--;
		i++;
	} else xmin = -3.0;

	if (argc > 1) {
		xmax = strtod(argv[i], &aux);
		argc--;
		i++;
	} else xmax = 3.0;

	if (argc > 1) {
		step = strtod(argv[i], &aux);
		argc--;
		i++;
	} else step = 0.01;

	if (argc > 1) {
		NR = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else NR = 1000;

	if (argc > 1) {
		thrds = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else thrds = 0;

	if (argc > 1) {
		abstol = strtod(argv[i], &aux);
		argc--;
		i++;
	} else abstol = 1.2e-12;

	if (argc > 1) {
		reltol = strtod(argv[i], &aux);
		argc--;
		i++;
	} else reltol = 1.2e-8;

	if (argc > 1) {
		method1 = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else method1 = STABLE_QNG;

	if (argc > 1) {
		method2 = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else method2 = STABLE_QAG2;

	if (argc > 1) {
		F = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else F = 15;

	if (argc > 1) {
		C = (int)strtod(argv[i], &aux);
		argc--;
		i++;
	} else C = 70;
